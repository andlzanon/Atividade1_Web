...
public class NovoSiteServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        NovoSiteFormBean nsfb = new NovoSiteFormBean();
        try{
           BeanUtils.populate(nsfb, request.getParameterMap());
           request.getSession().setAttribute("novoSite", nsfb);
           List<String> mensagens = nsfb.validar();
            if (mensagens == null) {
                request.getRequestDispatcher("confirmarNovoSite.jsp").forward(request, response);
            } else {
